...
def plot_GBM_paths(
    ...
    for i in range(iterations):
        _, s = GBM_plot(S0=S0, r=r, q=q, T=T, sigma=sigma, time_horizon=time_horizon)
        paths[i, :] = s
    ...
    se_payoff = payoff_std / np.sqrt(N)
    V = discount_factor * payoffs.mean()
    se_V = discount_factor * se_payoff
    ...
    plt.fill_between(t, lower, upper, alpha=0.3, label="95% distribution band (2.5–97.5 pct)")
    plt.plot(t, median_path, linewidth=3, label="Median path")
    plt.plot(t, mean_path, linestyle="-", linewidth=3, label="Mean path")
    plt.xlabel("Time (years)")
    plt.ylabel("Price ($)")
    plt.legend()
    plt.show()

    _, _, price = bs_price(S0, K, r, q, T, sigma)
